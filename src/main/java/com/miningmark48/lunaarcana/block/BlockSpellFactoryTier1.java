...
public class BlockSpellFactoryTier1 extends BlockContainer{

    public BlockSpellFactoryTier1(Material material){
        super(material);
    }
    ...
    public TileEntity createNewTileEntity(World par1, int par2) {
        return new TileEntitySpellFactoryTier1();
    }
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ){
        if (!player.isSneaking()){
            player.openGui(Reference.MOD_ID, GUIs.guiSFTier1, world, x, y, z);
            return true;
        }else{
            return false;
        }
